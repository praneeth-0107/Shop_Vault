...
async function placeOrder() {
  ...
  if (payData.mode === 'simulation') {
    // Simulation mode — skip Razorpay popup, directly verify
    await handleSimulatedPayment(orderId, payData);
  } else {
    // Live mode — open real Razorpay Checkout
    openRazorpayCheckout(orderId, payData);
  }
}

/**
 * Handle simulated payment (when no real Razorpay keys are configured)
 * Directly calls the verify endpoint which auto-accepts in simulation mode
 */
async function handleSimulatedPayment(orderId, payData) {
  showToast('Processing payment...', 'info');

  const { ok, data } = await apiPost('/api/payments/verify', {
    orderId,
    razorpayOrderId: payData.razorpayOrder.id,
  ...
  if (ok) {
    ...
    showToast(data.message || 'Payment failed. Contact support.', 'error');
    setTimeout(() => window.location.href = '/orders', 2000);
  }
}
