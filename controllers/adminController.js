  ...
  try {
    const [customers] = await pool.execute(
      `SELECT c.Customer_ID, c.Name, c.Email, c.Mobile_No, c.Address, c.Created_At,
              ...
              (SELECT COALESCE(SUM(o.Total_Amount), 0) FROM orders o INNER JOIN payments p ON o.Order_ID = p.Order_ID WHERE o.Customer_ID = c.Customer_ID AND p.Payment_Status = 'Completed' AND o.Order_Status != 'Cancelled') as Total_Spent
       FROM customers c
...
async function getDashboardSummary(req, res) {
  try {
    ...
    const [totalRevenue] = await pool.execute(
      "SELECT COALESCE(SUM(o.Total_Amount), 0) as revenue FROM orders o INNER JOIN payments p ON o.Order_ID = p.Order_ID WHERE p.Payment_Status = 'Completed' AND o.Order_Status != 'Cancelled'"
    );
...
async function getSalesReport(req, res) {
  try {
    ...
    let query = `
      ...
      WHERE p.Payment_Status = 'Completed' AND o.Order_Status != 'Cancelled'
    `;
    const params = [];
    ...
    const [topProducts] = await pool.execute(
      `SELECT p.Product_Name, p.Category, SUM(oi.Quantity) as total_sold, SUM(oi.Price * oi.Quantity) as total_revenue
       ...
       WHERE pay.Payment_Status = 'Completed' AND o.Order_Status != 'Cancelled'
